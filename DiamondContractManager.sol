...
library DiamondContractManager {
    ...
    function replaceFunctions(Data storage $, address _facet, bytes4[] memory _functs) internal {
        ...
        for (uint i; i < _functs.length; ++i) {
            ...
            if (old.facet == address(0)) revert IDiamond.CannotReplaceFunctionThatDoesNotExists(_functs[i]);
            ...
            if ($.facet[old.facet].functs.length > 1) {
                ...
                $.facet[old.facet].functs[old.position - 1] = $.facet[old.facet].functs[last];
                $.funct[$.facet[old.facet].functs[last]].position = old.position;
                $.facet[old.facet].functs.pop();
            } else {
                $.facets[$.index[old.facet] - 1] = $.facets[$.facets.length - 1];
                $.facets.pop();
        ...
        for (uint i; i < _functs.length; ++i) {
            ...
            if ($.facet[_facet].functs.length > 1) {
                if (old.position != position) {
                    ...
                    $.facet[_facet].functs[old.position - 1] = last;
