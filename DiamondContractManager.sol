...
library DiamondContractManager {
    ...
    function enforceIsContractOwner() internal view {
        Data storage $ = diamond(base);
        if ($.owner != msg.sender) revert IDiamond.NotContractOwner(msg.sender, $.owner);
    }
    ...
    function checkAccess(bytes32 _key, address _owner) internal view returns (bool check) {
    ...
    function functs(bytes32 _key, address _facet) internal view returns (bytes4[] memory _functs) {
        _functs = diamond(_key).facet[_facet].functs;
        return _functs.length == 0 ? diamond(base).facet[_facet].functs : _functs;
    }

    function facet(bytes32 _key, bytes4 _funct) internal view returns (address _facet) {
        _facet = diamond(_key).funct[_funct].facet;
        return _facet == address(0) ? diamond(base).funct[_funct].facet : _facet;
    }

    function facets(bytes32 _key) internal view returns (address[] memory _facets) {
    ...
    function getFacets(bytes32 _key) internal view returns (Facet[] memory facets_) {
        Data storage $ = diamond(_key);
        ...
        facets_[length] = Facet(address(this), diamond(base).facet[address(this)].functs);
    }

    function setInterface(bytes32 _key, bytes4 _interface, bool _state) internal {
        checkAccess(_key, msg.sender);
        diamond(_key).interfaces[_interface] = _state;
    }

    function checkInterface(bytes32 _key, bytes4 _interface) internal view returns (bool) {
        return diamond(_key).interfaces[_interface];
    }

    /* DiamondCut */

    event DiamondCut(IDiamond.Data[] _diamondCut, address _init, bytes _calldata);

    event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);

    // Internal function version of diamondCut
    function diamondCut(IDiamond.Cut[] memory _diamondCut, address _init, bytes memory _calldata) internal {
        for (uint i; i < _diamondCut.length; ++i) {
            ...
            for (uint j; j < _diamondCut[i].data.length; ++j) {
                bytes4[] memory functs_ = _diamondCut[i].data[j].functionSelectors;
                address facet_ = _diamondCut[i].data[j].facetAddress;
                if (functs_.length == 0) revert IDiamond.NoSelectorsProvidedForFacetForCut(facet_);
                IDiamond.Action action = _diamondCut[i].data[j].action;
                if (action == IDiamond.Action.Add) $.addFunctions(facet_, functs_);
                else if (action == IDiamond.Action.Replace) $.replaceFunctions(facet_, functs_);
                else if (action == IDiamond.Action.Remove) $.removeFunctions(facet_, functs_);
    ...
    function internalCut(bytes4[] memory _functs, bytes memory _calldata) internal {
    ...
    function _addFunctions(Data storage $, address _facet, bytes4[] memory _functs, bool _internal) internal {
        ...
        for (uint i; i < _functs.length; ++i) {
            if ($.funct[_functs[i]].facet != address(0)) {
                if (!_internal) revert IDiamond.CannotAddFunctionToDiamondThatAlreadyExists(_functs[i]);
    ...
    function addFunctions(Data storage $, address _facet, bytes4[] memory _functs) internal {
        if (_facet == address(0)) revert IDiamond.CannotAddSelectorsToZeroAddress(_functs);
        enforcedFacetHasCode(_facet, "DiamondCut: Add facet has no code");
        $._addFunctions(_facet, _functs, false);
    }

    function replaceFunctions(Data storage $, address _facet, bytes4[] memory _functs) internal {
        if (_facet == address(0)) revert IDiamond.CannotReplaceFunctionsFromFacetWithZeroAddress(_functs);
        enforcedFacetHasCode(_facet, "DiamondCut: Replace facet has no code");
        uint16 position = uint16($.facet[_facet].functs.length);
        if (_functs.length > 0 && position > 0) {
            for (uint i; i < _functs.length; ++i) {
                Funct memory old = $.funct[_functs[i]];
                if (old.facet == _facet) revert IDiamond.CannotReplaceFunctionWithTheSameFunctionFromTheSameFacet(_functs[i]);
                if (old.facet == address(0)) {
                    $.funct[_functs[i]].facet = _facet;
                    $.facet[_facet].functs.push(_functs[i]);
                }
                // can't replace immutable functions -- functions defined directly in the diamond in this case
                if (old.facet == address(this)) revert IDiamond.CannotReplaceImmutableFunction(_functs[i]);

                // delete old functions
                if ($.facet[old.facet].functs.length > 1) {
                    uint last = $.facet[old.facet].functs.length - 1;
                    $.facet[old.facet].functs[old.position] = $.facet[old.facet].functs[last];
            ...
            $.facets.push(_facet);
        } else if (_functs.length > 0) $.addFunctions(_facet, _functs);
        else revert IDiamond.ConnotReplaceWithWrongArguments(_facet);
    }

    function removeFunctions(Data storage $, address _facet, bytes4[] memory _functs) internal {
        uint position = $.facet[_facet].functs.length;
        if (position == 0) revert IDiamond.CannotRemoveFunctionFromFacetAddressThatDoesNotExist(_facet);
        for (uint i; i < _functs.length; ++i) {
            bytes4 funct_ = _functs[i];
            Funct memory old = $.funct[funct_];
            if (old.facet == address(0)) revert IDiamond.CannotRemoveFunctionThatDoesNotExist(funct_);
            // can't remove immutable functions -- functions defined directly in the diamond
            if (old.facet == address(this)) revert IDiamond.CannotRemoveImmutableFunction(funct_);
    ...
    function enforcedFacetHasCode(address _facet, string memory _errorMessage) internal view {
        uint size;
        assembly {
            size := extcodesize(_facet)
        }
        if (size == 0) revert IDiamond.NoBytecodeAtAddress(_facet, _errorMessage);
    }
}
